...
class ComputeStimTable(ReportRamTask):
    def __init__(self, params, mark_as_completed=True):
        super(ComputeStimTable,self).__init__(mark_as_completed)
        self.params = params
    ...
    def restore(self):
        ...
        self.stim_params_to_sess = joblib.load(self.get_path_to_resource_in_workspace('-'.join([subject,task,'stim_params_to_sess.pkl'])))
        self.stim_table = pd.read_pickle(self.get_path_to_resource_in_workspace('-'.join([subject, task, 'stim_table.pkl'])))
        self.pass_object('stim_params_to_sess', self.stim_params_to_sess)
        self.pass_object('stim_table', self.stim_table)
    ...
    def run(self):
        ...
        self.stim_table['item'] = events['item_name']
        self.stim_table['session'] = events.session
        self.stim_table['list'] = events.trial
        self.stim_table['serialpos'] = events.chestNum
        #self.th_stim_table['itemno'] = events.itemno
        self.stim_table['is_stim_list'] = [(s == 1) for s in events.stim_list]
        self.stim_table['is_stim_item'] = is_stim_item
        self.stim_table['is_post_stim_item'] = is_post_stim_item
        self.stim_table['recalled'] = recalls
        self.stim_table['prob'] = th_stim_prob
        self.stim_table['distance_err'] = events.distErr
        self.stim_table['confidence'] = events.confidence
        ...
        self.stim_table['stimAnodeTag'] = stim_anode_tag
        self.stim_table['stimCathodeTag'] = stim_cathode_tag
        self.stim_table['Region'] = region
        self.stim_table['Pulse_Frequency'] = pulse_frequency
        self.stim_table['Amplitude'] = amplitude
        self.stim_table['Duration'] = pulse_duration
        self.stim_table['Burst_Frequency'] = burst_frequency
        self.stim_table['auc'] = auc
        self.stim_table['auc_perm'] = auc_p
        ...
        subject=self.pipeline.subject
        task = self.pipeline.task
        joblib.dump(self.stim_params_to_sess,self.get_path_to_resource_in_workspace('-'.join([subject,task,'stim_params_to_sess.pkl'])))
        self.stim_table.to_pickle(self.get_path_to_resource_in_workspace('-'.join([subject, task, 'stim_table.pkl'])))
