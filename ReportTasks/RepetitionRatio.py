...
class RepetitionRatio(RamTask):
    ...
    def restore(self):
        subject = self.pipeline.subject
        self.repetition_ratios = joblib.load(
    ...
    def initialize_repetition_ratio(self):
        jreader = JsonIndexReader('/protocols/r1.json')
        subjects = jreader.subjects(experiment='catFR1')
        ...
        for subject in subjects:
            try:
                ...
                evs_field_list = ['item_num', 'serialpos', 'session', 'subject', 'rectime', 'experiment', 'mstime',
                                  'type',
                                  'eegoffset', 'iscorrect', 'answer', 'recalled', 'item_name', 'intrusion', 'montage',
                                  'list',
                                  'eegfile', 'msoffset']
                ...
                event_files = sorted(
                    list(json_reader.aggregate_values('all_events', subject=subj_code, montage=montage,
                                                      experiment='catFR1')))
                events = None
